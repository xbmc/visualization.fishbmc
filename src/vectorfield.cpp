...
static inline void _fische__vectorfield_randomize_(fische__vector* vec)
{
...
static inline void _fische__vectorfield_validate_(struct _fische__vectorfield_* P,
                                                  fische__vector* vec,
                                                  double x,
                                                  double y)
{
  ...
  for (y = params->start_y; y < params->end_y; y++)
  {
    ...
    for (x = 0; x < P->width; x++)
    {
      ...
      switch (fieldno)
      {
        ...
        case 7:
          // purely random
          v.x = rand_r(&rand_seed) % (int_fast32_t)(2 * speed * corr[fieldno] + 1) -
                (speed * corr[fieldno]);
          v.y = rand_r(&rand_seed) % (int_fast32_t)(2 * speed * corr[fieldno] + 1) -
                (speed * corr[fieldno]);
          break;
        ...
        case 17:
        {
          ...
          v = n;
          fische__vector_mul(&v,
                             sin(24 * fische__vector_angle(&e)) * -speed * 0.33 * corr[fieldno]);
          fische__vector_add(&v, &_v1);
          break;
        }
...
void _fische__fill_field_(struct _fische__vectorfield_* P, uint_fast8_t fieldno)
{
                                                    ...
                                                    uint_fast8_t* cancel)
{
  ...
  struct fische__vectorfield* retval =
      static_cast<fische__vectorfield*>(malloc(sizeof(struct fische__vectorfield)));
