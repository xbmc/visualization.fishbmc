...
/**
 * @brief Own for Kodi created function to get direct a hash hexadecimal string.
 */
std::string hash_as_hex(const void* input_bs, uint64_t input_size)
{
  void* hash = hash_bs(input_bs, input_size);
  if (hash == nullptr)
    return "";

  const std::string ret = sig2hex(hash);
  delete[] static_cast<uint8_t*>(hash);
  return ret;
}
