...
{
  _FISCHE__WAITING_,
  _FISCHE__MAYBEWAITING_,
  _FISCHE__BEAT_
};

int _fische__compare_int_fast16_t_(void const* value1, void const* value2)
{
  return (*(int_fast16_t*)value1 - *(int_fast16_t*)value2);
}

double _fische__guess_frames_per_beat_(uint_fast16_t* beat_gap_history)
{
...
double _fische__get_audio_level_(double* data, uint_fast32_t data_size)
{
  double E = 0;
...
struct fische__analyst* fische__analyst_new(struct fische* parent)
{
  ...
  struct fische__analyst* retval =
      static_cast<fische__analyst*>(malloc(sizeof(struct fische__analyst)));
...
int_fast8_t fische__analyst_analyse(struct fische__analyst* self, double* data, uint_fast16_t size)
{
  ...
  switch (P->state)
  {
    case _FISCHE__WAITING_:
      ...
      P->intensity_std_dev =
          P->intensity_std_dev * 0.95 + fabs(P->intensity_moving_avg - relative_intensity) * 0.05;
      ...
      if ((self->frames_per_beat) && (self->frames_per_beat / new_frames_per_beat < 1.2) &&
          (new_frames_per_beat / self->frames_per_beat < 1.2))
